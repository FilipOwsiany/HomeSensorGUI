...
void CMainViewManager::registerView(lv_obj_t * aView, uint32_t aId, const std::string& aName)
{
    printf("CMainViewManager::registerView id: %d, ptr: %p, name: %s\n", aId, aView, aName.c_str());
    SObjectContairner<lv_obj_t> container;
...
void CMainViewManager::callback(lv_event_t * aEvent, uint32_t aId)
{
    ...
    auto itView = std::find_if(mViewContainers.begin(), mViewContainers.end(),
                        [aId](const SObjectContairner<lv_obj_t>& c){
                            return c.mId == aId;
                        });
    ...
    if (itView != mViewContainers.end()) 
    {
        for (auto& container : mViewContainers) 
        {
            ...
            {
                lv_obj_clear_flag(container.mObject, LV_OBJ_FLAG_HIDDEN);
            } 
            else 
            {
                lv_obj_add_flag(container.mObject, LV_OBJ_FLAG_HIDDEN);
            }
        }
    } 
}